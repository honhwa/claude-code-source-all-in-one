...
export async function launchRepl(root: Root, appProps: AppWrapperProps, replProps: REPLProps, renderAndRun: (root: Root, element: React.ReactNode) => Promise<void>): Promise<void> {
  ...
  fs.appendFileSync('/tmp/cc-debug.log', '[REPL] importing App...\n');
  const {
    App
  } = await import('./components/App.js');
  fs.appendFileSync('/tmp/cc-debug.log', '[REPL] importing REPL...\n');
  const {
    REPL
  } = await import('./screens/REPL.js');
  fs.appendFileSync('/tmp/cc-debug.log', '[REPL] calling renderAndRun...\n');
  await renderAndRun(root, <App {...appProps}>
      <REPL {...replProps} />
    </App>);
  fs.appendFileSync('/tmp/cc-debug.log', '[REPL] renderAndRun returned\n');
}

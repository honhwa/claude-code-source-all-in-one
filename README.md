...
| **Anthropic API Key** | A valid `sk-ant-xxx` key |
...
- **Launcher**: `start.sh` is macOS/Linux only.
...
| `start.sh` | macOS/Linux launcher with first-run detection, API key validation, proxy auto-detection |
...
| `ANTHROPIC_API_KEY is not set` | `export ANTHROPIC_API_KEY="sk-ant-xxx"` |
...
- **启动脚本**：`start.sh` 仅适用于 macOS/Linux。
...
| `start.sh` | macOS/Linux 启动脚本，含首次运行检测、API Key 校验和第三方代理自动检测 |

...
本仓库支持从源码本地启动 Claude Code（功能有所缩减）。持续跟随上游发布，当前已对齐 `2.1.123`（详见 [`CHANGELOG.md`](CHANGELOG.md)）。
...
<details>
<summary><strong>备选方案：使用 ChatGPT 订阅（GPT-5.5）</strong></summary>

如果你有 ChatGPT Plus/Pro/Business/Enterprise 订阅，可以直接使用 GPT-5.5 作为 Agent 模型，无需 API Key：
...
<p align="center">
  <img src="assets/chatGPT-sub.png" alt="通过 ChatGPT 订阅运行 GPT-5.5" width="100%">
  <br>
  <em>通过 ChatGPT 订阅使用 GPT 作为 Agent 模型</em>
</p>
...
| `gpt5.4` | `chatgpt:gpt-5.4` | Plus / Pro / Business / Enterprise（旧版） |
| `gpt5.4-mini` | `chatgpt:gpt-5.4-mini` | 任意付费订阅 |
| `gpt5.4-nano` | `chatgpt:gpt-5.4-nano` | 任意付费订阅 |

也可以使用完整格式：`--model chatgpt:gpt-5.5`
...
<details>
<summary><strong>备选方案：使用本地 Ollama 模型（离线 / 无需 API Key）</strong></summary>

完全在本地 Ollama 模型上运行 Claude Code —— 无需 API Key，不向 Anthropic 或 OpenAI 发起任何请求。启动器会拉起内置的 Anthropic→Ollama 代理（[`scripts/ollama-proxy.mjs`](scripts/ollama-proxy.mjs)），把 CLI 指向它。
...
# 3. 在本地模型上启动 Claude Code
./start-ollama.sh qwen2.5-coder:7b
```

如果模型没有原生工具调用能力，设置 `PROMPTED_TOOLS=1`：
